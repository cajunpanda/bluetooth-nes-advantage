...
Retries are bounded in two tiers: 4 fast attempts, each paced by the page's own completion and so
about 5 s apart, then a slow re-page every 30 s, 6 of those, then passive. The slow tier covers a
host that is not listening yet. A stick powered on before the console or dock spends all four fast
attempts in the first ~20 s, and a host that accepts only controller-initiated reconnects then
waits for a page that never comes. In gameplay the app's 90 s unconnected deep sleep usually ends
the sequence before the slow budget does; waking reboots and pages from the top.
